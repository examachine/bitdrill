 ...
 * George
 *
 * $Id: wnodebisect.c 2668 2007-09-13 20:20:48Z exa $
 *
 */
...
void METIS_WNodeBS(int *nvtxs, idxtype *xadj, idxtype *adjncy, idxtype *vwgt,
		   int *numflag, int *options, idxtype *perm, idxtype *iperm)
{
  ...
  if (options[0] == 0) {  /* Use the default parameters */
    ...
    ctrl.IType   = IPART_GGPKLNODE; /* initial partitioning type */
  ...
  if (CheckGraph(&graph))
    ...
    printf("Nvtxs: %6d, [A %d:%6d, B %d:%6d], S %d:%6d]\n",
	   graph.nvtxs,
	   count_part(&graph,0), graph.pwgts[0],
	   count_part(&graph,1), graph.pwgts[1],
	   count_part(&graph,2), graph.pwgts[2]);
...
main(int argc, char *argv[])
{
  ...
  starttimer(METISTmr);
  METIS_WNodeBS(&graph.nvtxs, graph.xadj, graph.adjncy, graph.vwgt,
