 ...
 * George
 *
 * $Id: nodebisect.c 1135 2003-09-22 18:08:32Z exa $
 *
 */

#include <metis.h>

void METIS_NodeBS(int *nvtxs, idxtype *xadj, idxtype *adjncy, int *numflag, int *options, 
...
main(int argc, char *argv[])
{
  ...
  if (graph.ncon != 1) {
    printf("Node bisection can only be applied to graphs with one constraint.\n");
    exit(0);
  }
  stoptimer(IOTmr);
  ...
  starttimer(METISTmr);
  METIS_NodeBS(&graph.nvtxs, graph.xadj, graph.adjncy, &numflag, options, perm, iperm);
  stoptimer(METISTmr);
