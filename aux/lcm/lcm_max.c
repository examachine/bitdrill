...
QUEUE_INT *LCMmax_shrink(ARY *T, QUEUE *jump, long *p, int max_item, int cnt){
  ...
  malloc2 ( buf, QUEUE_INT, cnt, "LCMmax_shrink", "buf");
  b = buf;
...
void LCMmax_mk_weighted_trsact( ARY *T, int cnt ){
  ...
  malloc2 ( buf, QUEUE_INT, cnt, "LCMmax_mk_weighted_trsact", "buf");
  q = Q->q;
  for ( i=0 ; i<T->num ; i++ ){
    b = buf;
    for ( j=0 ; j<Q[i].t ; j++ ){
...
int LCMmax_mk_freq_trsact( ARY *TT, ARY *T, ARY *O, QUEUE *occ, int cnt,
  ...
  ARY_exp ( TT, occ->t-1 );
  QQ = TT->h;
  malloc2 ( buf, QUEUE_INT, cnt, "LCMmax_mk_freq_trsact", "buf" );
  bbuf = buf;
  for ( t=(long *)(occ->q) ; t<tt ; t++ ){
    QQ[n].q = b = buf;
...
int *LCMmax_alloc_occ(ARY *T, ARY *O, ARY *OO, int cnt, QUEUE *jump, int *pm, int item){
  ...
  malloc2 ( perm, int, (jump->t-jump->s)*2, "LCMmax_alloc_occ", "perm");
  QQ = OO->h; Q = O->h;
...
int LCMmax_iter(ARY *T,ARY *O,int item, int *pm, int *pm2, int prv, QUEUE *jump){
  ...
  int qt = LCM_Qtmp.t, flag=-1, flag2, cc=0, itemt, itemt2, itemt22, ff=0;
  QUEUE_INT *q;
