...
#ifndef __DCICLOSED_ITEMS_H
#define __DCICLOSED_ITEMS_H
...
class dci_items {
 public:
  ...
  inline bool use_key_patterns() {
		//plog << "real:  " << avg_supp_at_threshold << endl;
		//plog << "thres: "<<AVG_SUPP_THRESHOLD << endl;
...
void set_of_itemsets<T, T1>::printf_itemsets() {
  ...
  plog << "k=" << k  << endl;
  while (1) {
    plog << "Prefix:" << curr_prefix << "- Section len:" 
...
class close_item {
 public:
	 /*
  close_item(T i=0, int c=0, unsigned int* t=NULL, bool inc=false, int n=0)
	  {	id=i; count=c; head_list = t; 
			isSectionIncluded = inc;
		  sect_eq_num_bit = n;}*/
  T id;
  int count;
	unsigned int* head_list;
	unsigned int* tail_list;
...
class POST_item {
 public:
	POST_item (T i=0, /*int c=0,*/ unsigned int inc=0) {id = i; /*count=c;*/ included=inc; }
  T id;
//  int count;
	unsigned int included;
};
...
class close_itemset {
 public:
	 /*
  close_itemset(int c=0, unsigned int* t=NULL, bool inc=false,  int n=0)
	  {	count=c; head_list = t; 
			isSectionIncluded = inc; 
		  sect_eq_num_bit = n; lung=0;}*/
  T* items;
	unsigned int lung;
  int count;
	unsigned int* head_list;
