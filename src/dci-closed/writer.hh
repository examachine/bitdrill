...
#ifndef __DCICLOSED_WRITER_H
#define __DCICLOSED_WRITER_H
...
public:
  /** counstructor */
  ItemsetWriter(dci_items* ii, const char* file_name);
  /** destructor */
  ~ItemsetWriter();

  void Write(close_itemset<T> &itemset);
...
private:
  ...
  static const int SZ_NUM = 16;
  FSout* out;
};

template <class T>
ItemsetWriter<T>::ItemsetWriter(dci_items* ii, const char* file_name){
  counters = ii;
...
template <class T>
void ItemsetWriter<T>::Write(close_itemset<T> &itemset) { 
  unsigned int div;
  int ind, num_written;
