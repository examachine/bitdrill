...
#ifndef __KDCI_DATABASE_H
#define __KDCI_DATABASE_H
...
public:
  ...
  inline bool getNextTransaction(KDCI::Transaction<unsigned int>& t);
...
inline bool Data::getNextTransaction(vector<unsigned int>& t)
{
  ...
  /*
inline bool Data::getNextTransaction(KDCI::Transaction<unsigned int>& t)
{
  ::Transaction ti;
  bool ret = scanner->read(ti);
  t.length = ti.size();
...
class binFSout
{
 public:
  ...
  inline void writeTransaction(KDCI::Transaction<T>& t);
...
class binFSin
{
 public:
  ...
  int getNextTransaction(KDCI::Transaction<T>& t);
...
template <class T>
inline void binFSout<T>::writeTransaction(KDCI::Transaction<T>& t)
{
  if (t.length != 0) {
    n_tr++; 
...
template <class T>
int binFSin<T>::getNextTransaction(KDCI::Transaction<T>& t)
{
  int ret;
