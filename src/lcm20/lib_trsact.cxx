...
ARY TRSACT_file_count ( Transaction_Set& ts, int *Tnum ){
  ...
  *Tnum=0; /* # transactions */
  ...
  while (!scanner->end()) {
    Transaction t;
    scanner->read(t);
    for (Transaction::iterator tit = t.begin(); tit!=t.end(); tit++) {
      int item = *tit;
...
/***********************************/
int *TRSACT_load ( Transaction_Set &ts, ARY *T, int th ){
  QUEUE *Q;
  ARY E;
  ...
  T->num = Tnum;
  T->dellist = Enum;   /* store max_item: illegal use */
  free ( buf );
  ARY_end ( &E );
  ts.clear();
...
/*************************************************************************/
int LCM_init ( Transaction_Set& ts, int threshold, const char* out_fn=0 ){
  int i, n=0, m;
  QUEUE *Q;
  ...
  LCM_th = threshold;
  LCM_print_flag |= (out_fn!=0? 1: 0);
  LCM_perm = TRSACT_load ( ts, &LCM_Trsact, LCM_th );
  ...
  if ( LCM_print_flag&1 ) fprint_int_init ( out_fn );
  return ( n );
}
...
/* Common initialization for LCM, LCMfreq, LCMmax */
/*************************************************************************/
int LCM_init_cli ( int argc, char *argv[] ){
  int i, n=0, m;
  QUEUE *Q;
  ...
  Transaction_Set ts(argv[1]);
  if (LCM_print_flag)
    return LCM_init(ts, LCM_th, argv[3]);
  else
    return LCM_init(ts, LCM_th);
}
